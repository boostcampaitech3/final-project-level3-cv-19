...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = BaseConv(in_channels, in_channels * 5, 1, stride=1, act=act)
        self.conv2 = Conv(in_channels *5, in_channels *5, 3, stride=1, act=act)
        self.conv3 = Conv(in_channels * 5 , out_channels, 1, stride=1, act=act)
        self.use_add = shortcut and in_channels == out_channels


    def forward(self, x):
        y = self.conv3((self.conv2(self.conv1(x)))) # relu는 나중에 넣어보자
        if self.use_add:
            y = y + x
        return y
